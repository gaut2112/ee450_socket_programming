...
local_addr.sin_port = htons(0);
 ...
 // print our own socket and IP address code taken from beej
 int i;
 bool hst;
 struct hostent *he;
...
addr_list = (struct in_addr **)he->h_addr_list;
	...
	{
        cout<<"Phase 1: Patient 2 has TCP port Number "<<ntohs(local_addr.sin_port)<<" and IP address "<<inet_ntoa(*addr_list[i])<<endl;
    }
//initialize
...
cout<<"End of Phase 1 for Patient 2"<<endl;
if(authe == 1)
 {
  cout<<"Phase 2 :Following Appointments are available for Patient 2"<<endl;
  ...
  strtok(a_i," ");
  //cout<<app_info<<endl;
  cout<<"Phase 2: The requested appointment is available and reserved for patient2. The assigned doctor port number is "<<(atoi(strtok(NULL," "))+851)<<endl;
...
cout<<"Phase 3:Patient 2 has dynamic UDP port number "<<ntohs(myaddr.sin_port)<<" and IP address "<<inet_ntoa(*addr_list[0])<<endl;
...
//send doctor data
fstream f("patient2insurance.txt",ios::in);
f.getline(ins,20);
f.close();
cout<<"Phase 3:The cost estimation request from Patient2 with insurance plan "<<ins<<" has been sent to doctor with port number "<<prt<<" and IP address "<<inet_ntoa(*addr_list[0])<<endl;
sendto(udp_sock,ins,strlen(ins)+1,0,(sockaddr *)&docaddr,sizeof(docaddr));
//receive from server
recvfrom(udp_sock,cost,3,0,(sockaddr *)&docaddr,&si);
cout<<"Phase 3:Patient 2 receives cost estimation of "<<cost<<"$ from doctor with port number"<<p<<" and name"<<doc_name<<endl<<"End of phase 3 for patient 2";
close(udp_sock);
}
else
 {
...
{
cout<<"Phase 1:Authentication Request from Patient2 with username "<<user<<" and password "<<pass<<" has been sent to Health Centre Server"<<endl;
...
cout<<"Phase1: Patient 2 authentication result "<<rec<<endl;
...
{
fstream credentials ("patient2.txt", ios::in);
