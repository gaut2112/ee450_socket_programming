...
addr_list = (struct in_addr **)he->h_addr_list;
	...
	{
        cout<<"Phase 3:Doctor 2 has static UDP port "<<PORT<<" and IP address "<<inet_ntoa(*addr_list[i])<<endl;
    }
...
//start receiving from client : following code has been is modified version of Bidirectional UDP code from https://www.cs.rutgers.edu/~pxk/417/notes/sockets/udp.html  //
 ...
 { 
   ...
   cout<<"Phase 3:Doctor 2 receives a request from patient with port number "<<ntohs(remote.sin_port)<<" and the insurance plan "<<buffer<<endl;
   calculateInsurance(strtok(buffer," "),ins_m,ins_send);
   
   sendto(sockdes,ins_send,strlen(ins_send)+1,0,(sockaddr *)&remote,len);
   cout<<"Phase 3:Doctor 2 has sent estimated price of $"<<ins_send<<" to patient with port number "<<ntohs(remote.sin_port)<<endl;
  }
  return 0;
 }
